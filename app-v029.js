...
const sameOriginApiEnabled = typeof window !== "undefined" && (
...
function getHealthUrl() {
  return sameOriginApiEnabled ? new URL("api/health", window.location.href).toString() : "";
}
...
async function checkService() {
  if (!liveApiEnabled || !sameOriginApiEnabled) {
    state.cloudReachable = false;
    state.cloudGenerationMode = "local";
    setServiceStatus("fallback", "商用体验模式可用");
    return;
  }
...
async function runAccessDiagnostics(shouldToast = false) {
  ...
  if (!liveApiEnabled || !sameOriginApiEnabled) {
    ...
    apiRouteMeta.textContent = "当前入口优先保证客户可打开；正式域名部署后启用真实生成";
  } else {
    try {
