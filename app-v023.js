...
const emergencyMirrorEntry = "https://evolved-prata-m2me.pagedrop.io/";
...
const expectedCacheName = "short-drama-studio-v23";
const liveApiEnabled = false;
const customerEntries = [
  ["客户主入口", githubPagesEntry],
  ["备用入口 1（长链主站）", projectPagesEntry],
  ["备用入口 2（临时单页镜像，3 天）", emergencyMirrorEntry],
  ["备用入口 3（Cloudflare Worker）", primaryWorkerEntry]
];
...
async function copyText(text, successMessage = "已复制") {
  try {
    if (location.hostname.endsWith(".pagedrop.io") || !navigator.clipboard?.writeText) {
...
function buildCustomerEntryText() {
  return [
    ...
    "",
    "建议：如果微信内打不开，请先复制到手机 Chrome / Safari 打开；还不行就发备用入口 2。"
  ].join("\n");
}
...
function buildCustomerPitchText() {
  return [
    ...
    "",
    `如果微信里打不开，复制到手机 Chrome / Safari；还不行用临时单页镜像：${emergencyMirrorEntry}`
  ].join("\n");
}
